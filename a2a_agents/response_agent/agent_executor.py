...
This module contains the executor for the Response Agent, which handles the
actual execution of response generation tasks.
"""
...
class ResponseAgentExecutor(AgentExecutor):
    """Executor for the Response Agent."""
    
    def __init__(self, **kwargs):
        """Initialize the ResponseAgentExecutor."""
    ...
    async def execute(
        ...
        logger.info(f"\nExecuting response generation task: {query}")
        ...
        try:
            ...
            async for event in self.agent.generate_response(query, task.context_id):
